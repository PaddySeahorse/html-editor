...
export function assignIds(root: Root): Root {
  visit(root, 'element', (node: Element) => {
    ...
    if (!node.properties.dataEditorId) {
      node.properties.dataEditorId = generateId();
    }
  });
  return root;
}
...
export function buildIndexMaps(root: Root): NodeIndexMap {
  ...
  visit(root, 'element', (node: Element) => {
    const id = node.properties?.dataEditorId as string | undefined;
    if (id) {
      byId.set(id, node);
...
export function findNodeById(root: Root, id: string): Element | undefined {
  ...
  visit(root, 'element', (node: Element) => {
    if (node.properties?.dataEditorId === id) {
      found = node;
      return false;
    }
  });
