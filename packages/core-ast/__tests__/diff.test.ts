  ...
  describe('applyPatch', () => {
    it('should replace node by id', () => {
      ...
      if (div.type === 'element') {
        ...
        if (p.type === 'element') {
          const id = p.properties?.dataEditorId as string;

          const newP: Element = {
            type: 'element',
            tagName: 'p',
            properties: { dataEditorId: id },
  ...
  describe('detectChanges', () => {
    ...
    it('should return empty array when no structural changes', () => {
      ...
      if (div1.type === 'element') {
        ...
        if (p1.type === 'element') {
          const divId = div1.properties?.dataEditorId;
          const pId = p1.properties?.dataEditorId;

          const div2 = tree2.children[0];
          if (div2.type === 'element') {
            div2.properties = { ...div2.properties, dataEditorId: divId };
            const p2 = div2.children[0];
            if (p2.type === 'element') {
              p2.properties = { ...p2.properties, dataEditorId: pId };
            }
          }
        }
      }
