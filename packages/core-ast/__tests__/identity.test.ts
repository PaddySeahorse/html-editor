...
describe('identity', () => {
  ...
  describe('assignIds', () => {
    it('should assign data-id to all elements', () => {
      ...
      if (div.type === 'element') {
        expect(div.properties?.dataEditorId).toBeDefined();
        expect(typeof div.properties?.dataEditorId).toBe('string');

        const p = div.children[0];
        const span = div.children[1];

        if (p.type === 'element') {
          expect(p.properties?.dataEditorId).toBeDefined();
        }
        if (span.type === 'element') {
          expect(span.properties?.dataEditorId).toBeDefined();
        }
      }
    });
    ...
    it('should generate unique IDs', () => {
      ...
      if (div.type === 'element') {
        ...
        if (p1.type === 'element' && p2.type === 'element') {
          expect(p1.properties?.dataEditorId).not.toBe(p2.properties?.dataEditorId);
        }
      }
    });

    it('should not overwrite existing data-id', () => {
      const html = '<div data-editor-id="custom-id">Content</div>';
      const tree = parseHtml(html);
      assignIds(tree);

      const div = tree.children[0];
      if (div.type === 'element') {
        expect(div.properties?.dataEditorId).toBe('custom-id');
      }
    });
  });
  ...
  describe('findNodeById', () => {
    it('should find node by id', () => {
      ...
      const div = tree.children[0];
      if (div.type === 'element') {
        const id = div.properties?.dataEditorId as string;
